...
public class Client {
    ...
    public boolean request(String user, String type) throws IOException, ClassNotFoundException {
        ...
        if (foundUser.equals("NOT_FOUND")) {
            ...
            System.out.println(Color.red("You can't send chat request to yourself."));
            Log my_log = Log.getInstance();
            my_log.logger.info("You can't send chat request to yourself.");
            return false;
        }
//      String ip = foundUser.split(":")[0];
        ...
        if (type.equals("CHAT_REQUEST")){ // Regular chat request
            ...
            Log my_log = Log.getInstance();
            my_log.logger.info("sent CHAT_REQUEST to " + user);
            System.out.println(Color.yellow("Sent a request to " + user));
            ...
            Log my_log = Log.getInstance();
            my_log.logger.info("sent GROUP_INVITE to " + user);
            System.out.println(Color.yellow("Sent an invite to " + user));
        }
    ...
    public void updateListenToPeers() throws Exception {
        while (true) {
            ...
            if (!serverThread.gettingRequest) System.out.println(Color.blue("Enter contact username to chat or /logout to logout: "));
    ...
    private void logout() throws IOException {
        ...
        initializeNewSocket();
        Request request = new Request(userName, password, 5, portNumber);
        ObjectOutputStream objectOutputStream = new ObjectOutputStream(socket.getOutputStream());
        ...
        if (this.socket != null && !this.socket.isClosed() && this.socket.isConnected() && objectOutputStream != null) {
            ...
            try {
                ...
                System.out.println(Color.red("Logged out!"));
                Log my_log = Log.getInstance();
                my_log.logger.info("User: "+userName+" logged out.");
                this.userName = null;
                this.password = null;
        ...
        while (true) {
            ...
            if (choice.equals("1")) {
                ...
                if(userInfo.length > 1)
                    ...
                    System.out.println(Color.red("Invalid username and password."));
                    my_log.logger.info("Invalid username and password.");
                    continue;
                }
