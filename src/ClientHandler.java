...
public class ClientHandler implements Runnable {
        ...
        if (this.socket!=null&&this.datagramPacket==null) {
            ...
            try {
                ...
                if (request != null) {
                    ...
                    else if(request.getRequestType()==5){logoutProcess(request);}
                }
    ...
    private void logoutProcess(Request request) {

        registeredUsers.get(lookForUser(request.getUserName())).setOnline(false);
        System.out.println(Color.red("User: "+request.getUserName()+" logged out!"));
        Log my_log = Log.getInstance();
        my_log.logger.info("User: "+request.getUserName()+" logged out.");
        long threadId=registeredUsers.get(lookForUser(request.getUserName())).getConnectionCheckThreadId();
        ...
        registeredUsers.get(lookForUser(request.getUserName())).setConnectionCheckThreadId(-1L);
    }
    ...
    private void loginProcess(Request request) {
        ...
        if (userIndex!=-1 && ClientHandler.registeredUsers.get(userIndex).getPassword().equals(request.getPassword()) && !ClientHandler.registeredUsers.get(userIndex).getOnline()){
            ...
            try {
              ...
              Thread connectionCheckThread=new Thread(new Runnable() {
                    @Override
                    public void run() {
             ...
             //associate the thread id with corresponding user
                registeredUsers.get(lookForUser(request.getUserName())).setConnectionCheckThreadId(connectionCheckThread.getId());
